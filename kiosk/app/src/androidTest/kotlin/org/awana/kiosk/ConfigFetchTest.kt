...
class ConfigFetchTest {
    ...
    fun aConfigMatchingItsHashIsAccepted() = runBlocking {
        ...
        val result = ConfigFetch.fetch(
            ProvisioningBootstrap(url, Digests.sha256Hex(body.toByteArray())),
            target,
        )
    ...
    fun aConfigWithASubstitutedPinIsRefused() = runBlocking {
        val genuine = config()
        val genuineHash = Digests.sha256Hex(genuine.encode().toByteArray())
    ...
    fun aSingleAlteredByteIsEnoughToRefuseIt() = runBlocking {
        ...
        val genuineHash = Digests.sha256Hex(genuine.toByteArray())
    ...
    fun theHashIsOverTheBytesAsServedNotTheReparsedForm() = runBlocking {
        ...
        val result = ConfigFetch.fetch(
            ProvisioningBootstrap(url, Digests.sha256Hex(reEncoded.toByteArray())),
            target,
        )

        assertEquals(
            "this fixture no longer detects re-encoding",
            Digests.sha256Hex(body.toByteArray()),
            Digests.sha256Hex(reEncoded.toByteArray()),
        )
