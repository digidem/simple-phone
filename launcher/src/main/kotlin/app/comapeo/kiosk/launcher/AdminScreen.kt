...
import app.comapeo.kiosk.policy.LockTaskBreakService
import app.comapeo.kiosk.policy.Provisioner
import app.comapeo.kiosk.policy.SideloadFromUrl
import app.comapeo.kiosk.policy.WifiAdmin
...
private fun DeviceInfoPage(onBack: () -> Unit) {
    ...
    val rows = remember {
        buildList {
            ...
            add(R.string.info_comapeo_version to comapeoVersion(context))
...
private fun ChangePinPage(onBack: () -> Unit) {
    ...
    AdminPage(stringResource(R.string.admin_change_pin), onBack, TAG_ADMIN_CHANGE_PIN) {
        item {
            NewPinFields(
                onSubmit = { pin ->
                    store.update { it.copy(adminPinHash = app.comapeo.kiosk.policy.AdminPin.hash(pin)) }
...
private fun comapeoVersion(context: Context): String = runCatching {
    val info = context.packageManager.getPackageInfo(
        app.comapeo.kiosk.policy.Pins.COMAPEO_PACKAGE,
        0,
    )
    "${info.versionName} (${info.longVersionCode})"
